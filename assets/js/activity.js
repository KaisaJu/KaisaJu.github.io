(function () {
  ...
  function sizeList() {
    ...
    list.style.maxHeight = rows.length > 3 ? (rows[3].offsetTop - rows[0].offsetTop) + 'px' : '';
  }
  ...
  function render() {
    ...
    list.setAttribute('aria-label', zh ? '开源动态，可向下滚动查看更多' : 'Open-source activity, scroll for more');
  ...
  Promise.all(['https://api.github.com/users/KaisaJu', 'https://api.github.com/users/KaisaJu/events/public?per_page=100'].map(function (url) {
    ...
    profile = data[0];
    events = data[1].filter(function (e) { return ['PushEvent','CreateEvent','PullRequestEvent','IssuesEvent','ReleaseEvent'].includes(e.type) && e.repo && e.repo.name && Number.isFinite(Date.parse(e.created_at)); }).sort(function (a,b) { return Date.parse(b.created_at) - Date.parse(a.created_at); });
    state = 'ready';
